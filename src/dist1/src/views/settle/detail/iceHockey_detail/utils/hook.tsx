...
export function useDetailHook() {
  const dataList = reactive<SattleDataAPI.IceHockeyEvent[]>([]);
  ...
  const onSearch = async () => {
    const res: SattleDataAPI.getIceHockeyEventRes =
      await API.getIceHockeyEvents({
  ...
  const addEventRow = () => {
    ...
    const params: Partial<SattleDataAPI.IceHockeyEvent> = {
      eventTime: '',
      matchId: history.state.params.matchId,
    ...
    dataList.push(params as SattleDataAPI.IceHockeyEvent);
  };
  //- 结算局事件
  const settlementHandicap = async (
    row: SattleDataAPI.BasketBallEventsData
  ) => {
  ...
  const allSettleBtnClick = async () => {
    ...
    if (dataList.length < 3 || dataList.some(item => !item.id) || s1 === 0) {
      return message(t('请先完成局或加时赛结算'), { type: 'error' });
    }
    ElMessageBox.confirm(
      `
      <div class="text-center">
      ...
      try {
        ...
        const res = await API.allIceHockeySettlement({
          matchId: history.state.params.matchId
        });
  ...
  const checkNoMore = () => {
    ...
    if (dataList.length === 4 || (dataList.length === 3 && s1 !== 0)) {
      return false;
    }
    return true;
  };
